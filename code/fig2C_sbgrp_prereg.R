# Subgroup analysis: trial registration
...
# Pre-registration
sbgrp <- subgroup.analysis.mixed.effects(x = dsl, subgroups = infile$REGISTERED)
...
sbgrp_NoSpain <- subgroup.analysis.mixed.effects(x = dsl_NoSpain, subgroups = infile_NoSpain$REGISTERED)
out_NoSpain <- sbgrp_NoSpain$within.subgroup.results %>%
  ...
  mutate(between_group_p = sbgrp_NoSpain$subgroup.analysis.results$p,
         ...
         subgroup = "omit N.W.Spain")

comb <- bind_rows(out, out_NoSpain)

# Plot labels
helper <- tibble(
  subgroup = c("all studies", "omit N.W.Spain"),
...
labels <- c(
  ...
  `omit N.W.Spain` = paste0("omit N.W.Spain (n=", helper$k[helper$subgroup == "omit N.W.Spain"], ")"))
...
pA <- ggplot(comb, aes(x = SMD , y = levels)) +
  ...
  geom_text(aes(x = 0.55, y = 1.5, label = paste("p=", signif(between_group_p, digits = 2), sep = "")),
            size = 2.25) +
...
ggsave("./figs/meta_Fig2C.pdf", pA, height = 2, width = 2.7)
